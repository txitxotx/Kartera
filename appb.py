    ...
    with sqlite3.connect('database.db') as conn:
        ...
        if cursor.fetchone()[0] == 0:
            investments = [
                ...
                ("0P00000BQY.F", "Baskepensiones Bolsa Euro EPSV", 4.5678, 5000)
            ]
...
def tables():
    with sqlite3.connect('database.db') as conn:
        ...
        dca = investments[:3]
        renta_fija_variable = investments[3:6]
        cryptomonedas = investments[6:8]
        acciones = []
        baskepensiones = investments[8:9]

    return render_template('tables.html', dca=dca, renta_fija_variable=renta_fija_variable, acciones=acciones, cryptomonedas=cryptomonedas, baskepensiones=baskepensiones)

@app.route('/bank')
def bank():
    banks = [
        ("Kutxabank Nomina", 10139.66, 0, ""),
        ("Kutxabank Conjunta", 403.88, 0, ""),
        ("Trade Republic", 10050, 3, ""),
        ("Revolut", 300, 1.51, ""),
        ("Bit2Me", 1000, 0, ""),
        ("My Investor", 0, 0, "")
    ]
...
def pie_chart():
    ...
    # Segmentar los datos tal y como en tables.html
    dca = investments[:3]
    renta_fija_variable = investments[3:6]
    cryptomonedas = investments[6:8]
    acciones = []  # Tal como se define en tables.html
    baskepensiones = investments[8:9]

    total_dca = sum(inv[6] for inv in dca)
    total_renta = sum(inv[6] for inv in renta_fija_variable)
    total_crypto = sum(inv[6] for inv in cryptomonedas)
    total_acciones = sum(inv[6] for inv in acciones)
    total_epsv = sum(inv[6] for inv in baskepensiones)
    overall_total = total_dca + total_renta + total_crypto + total_acciones + total_epsv

    perc_dca = (total_dca / overall_total) * 100 if overall_total > 0 else 0
    perc_renta = (total_renta / overall_total) * 100 if overall_total > 0 else 0
    ...
    pie_labels = ["DCA", "Renta Fija & Variable", "Cryptomonedas", "Acciones", "EPSV"]
    # Usamos una paleta de colores premium y modernos:
    custom_colors = ['#FF6B6B', '#FFD93D', '#6BCB77', '#4D96FF', '#BC6FF1']
    pie_values = [perc_dca, perc_renta, perc_crypto, perc_acciones, perc_epsv]

    # Configurar la gráfica de pastel con texto en #00FF00
    ...
    # Configurar la gráfica de barras con texto en #00FF00 y eje Y con "%" al final
    fig_bar = go.Figure()
    for label, percentage, color in zip(pie_labels, [perc_dca, perc_renta, perc_crypto, perc_acciones, perc_epsv], custom_colors):
    ...
    # Datos para la tabla: lista de tuplas (color, label, size, percentage)
    data_list = list(zip(custom_colors, pie_labels, [total_dca, total_renta, total_crypto, total_acciones, total_epsv],
                         [perc_dca, perc_renta, perc_crypto, perc_acciones, perc_epsv]))
...
# --- Fin modificaciones para gráfica redonda ---
